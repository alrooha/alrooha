...
const Footer = () => {
  ...
  return (
    <motion.footer variants={fadeIn("up", 0.2)} initial="hidden" whileInView="show" className="bg-gray-50">
        ...
        >
          ...
          <motion.div variants={fadeIn("right", 0.4)} className="lg:col-span-4">
            <motion.div variants={fadeIn("down", 0.5)} className="flex items-center gap-1 mb-6">
              ...
              <span className="text-xl font-medium ml-1">Al-Rooha Trade</span>
            </motion.div>
            <motion.p variants={fadeIn("up", 0.6)} className="text-gray-600 mb-6">
              Al-Rooha is your trusted partner in global multi-commodity import-export trading. We connect suppliers and
              buyers worldwide with excellence and transparency.
            </motion.p>
                  ...
                  <motion.ul variants={fadeIn("up", 0.4)} className="space-y-3">
                    {links.map((link, index) => (
                      <motion.li key={index} variants={fadeIn("up", 0.1 * (index + 1))}>
                        <motion.a whileHover={{ x: 5 }} href={link.href} className="text-gray-600 hover:text-gray-900">
                          {link.name}
                        </motion.a>
                      </motion.li>
                    ))}
        ...
        <motion.div variants={fadeIn("up", 0.8)} className="border-t border-gray-200 mt-12 pt-8">
          ...
          >
            <motion.p variants={fadeIn("right", 1.0)} className="text-gray-600 text-sm">
              Copyright © {new Date().getFullYear()} Al-Rooha Trading. All rights reserved.
            </motion.p>
