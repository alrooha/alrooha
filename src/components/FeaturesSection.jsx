  ...
  const features = [
    {
      ...
      description: "We analyze your commodity requirements and market conditions to present tailored trading solutions",
    },
    {
      icon: "⚙️",
      title: "Coordinate Logistics",
      description:
        "We work with trusted logistics partners to arrange efficient shipping and customs clearance globally",
    },
    {
      icon: "🚀",
      title: "Execute Seamlessly",
      description: "From procurement to final delivery, we manage every aspect with transparency and reliability",
    },
  ]
    ...
    >
      <motion.div variants={fadeIn("up", 0.3)} className="text-center mb-12">
        <motion.h2 variants={textVariant(0.2)} className="text-3xl font-bold mb-4">
          How We Transform Your Trade
        </motion.h2>
        <motion.p variants={fadeIn("up", 0.4)} className="text-gray-600">
          Al-Rooha simplifies international commodity trading through expertise and trusted partnerships
        </motion.p>
      </motion.div>

      <motion.div variants={fadeIn("up", 0.5)} className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {features.map((feature, index) => (
          <motion.div key={index} variants={fadeIn("up", 0.3 * (index + 1))} className="flex flex-col items-center p-6">
            <motion.div
            ...
            <motion.p variants={fadeIn("up", 0.6 * (index + 1))} className="text-gray-500 text-center">
              {feature.description}
            </motion.p>
          </motion.div>
        ))}
